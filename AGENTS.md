...
- cidre does not declare per-class override traits. Application code subclasses with `define_obj_type!(Name(Base), Inner, CLS)` and, in an `#[objc::add_methods] impl Name` block, overrides methods with `#[objc::overrides(sel)]` (each gets a generated `super_*`; a fn without a receiver overrides a class method such as `+layerClass`; a fn taking `self` by value, `#[objc::overrides(initWithFrame:)] fn init_with_frame(self, ..) -> arc::R<Self> { let mut v = self.super_init_with_frame(..); ..; v }`, overrides an initializer and requires `Inner: Default` because framework-created instances carry the `+allocWithZone:` payload) and creates instances with any payload via `alloc_with(inner)` followed by an initializer: an inherited one through `arc::A::init_with(|v| v.init_with_frame(frame))`, or one declared on the type with `#[objc::init(sel)]` (see `cidre/examples/ns-custom-view`). Keep the `NSObject`-only form `define_obj_type!(Name + Trait, Inner, CLS)` for delegates.
